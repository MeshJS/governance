create table public.cardano_project_roles (
  ...
  constraint cpr_principal_type_check check (
    (
      principal_type = any (array['wallet'::text, 'nft_unit'::text])
    )
  ),
  ...
  constraint cpr_unit_shape_check check (
    (
      (unit is null)
      or (unit ~ '^[0-9a-fA-F]{58,200}$'::text)
    )
  )
...
create index IF not exists idx_cpr_unit on public.cardano_project_roles using btree (unit) TABLESPACE pg_default
where
  (principal_type = 'nft_unit'::text);

create unique INDEX IF not exists ux_cpr_project_role_unit on public.cardano_project_roles using btree (project_id, role, unit) TABLESPACE pg_default
where
  (principal_type = 'nft_unit'::text);