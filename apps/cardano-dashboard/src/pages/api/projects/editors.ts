...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
    ...
    if (req.method === 'GET') {
        ...
        if ((proj as { owner_address: string | null }).owner_address !== address) {
            res.status(403).json({ error: 'Only owner can view editors' });
            return;
        }
        ...
        res.status(200).json({ editors: data ?? [] });
        return;
    }
    ...
    if (req.method === 'POST') {
        ...
        if ((proj as { owner_address: string | null }).owner_address !== address) {
            res.status(403).json({ error: 'Only owner can add editors' });
            return;
        }
        ...
        if (currentCount >= 50) {
            res.status(400).json({ error: 'Too many editors on this project' });
            return;
        }
        ...
        res.status(201).json({ editor: data });
        return;
    }
    ...
    if (req.method === 'DELETE') {
        ...
        if ((proj as { owner_address: string | null }).owner_address !== address) {
            res.status(403).json({ error: 'Only owner can remove editors' });
            return;
        }
