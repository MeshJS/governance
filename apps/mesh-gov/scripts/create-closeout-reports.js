...
function getProjectInfo(readmeContent, fundNumber) {
  ...
  const projectMatches = currentFundContent.matchAll(
    /\| \*\*Project ID\*\* \| (\d+) \|[\s\S]*?\| \*\*Name\*\* \| (.*?) \|[\s\S]*?\| \*\*Status\*\* \| (.*?) \|[\s\S]*?\| \*\*Milestones completed\*\* \| (\d+)\/(\d+)/g
  );
...
async function main() {
  ...
  for (const fundDir of fundDirs) {
    ...
    for (const projectDir of projectDirs) {
      ...
      const mdFiles = fs.readdirSync(projectPath).filter(file => file.endsWith('.md'));
