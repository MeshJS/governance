...
export default function Contributors() {
  ...
  const globalEarliestDate = useMemo(() => {
    ...
    humanContributors.forEach((contributor: Contributor) => {
  ...
  const timelineDisplayBoundaries = useMemo(() => {
    ...
    if (timeWindow.preset === 'all') {
      ...
      const globalStartDate = globalEarliestDate
        ? new Date(globalEarliestDate).toISOString().split('T')[0]
        : null;
      return {
  ...
  const filteredSummaryMetrics = useMemo(() => {
    ...
    return getFilteredSummaryMetrics(humanContributors, startDate, endDate);
  }, [humanContributors, isOrgStats, timeWindowBoundaries]);
  ...
  const sortedContributors = useMemo(() => {
    ...
    const contributorsWithMetrics: ContributorWithMetrics[] = humanContributors.map(
      (contributor: Contributor) => {
  ...
  return (
    <div className={styles.container}>
        ...
        <div className={styles.chartsContainer}>
          <div className={styles.chartWrapper}>
            ...
            <p className={styles.chartSubtitle}>
              Monthly activity trends for top{' '}
              {isOrgStats ? Math.min(15, humanContributors.length) : 15} contributors
            </p>
      ...
      <div className={styles.contributorsGrid}>
        {sortedContributors
          .map((item: ContributorWithMetrics, index: number) => {
              ...
              >
                ...
                <div className={styles.timelineContainer}>
                  <ContributionTimeline
                    commitTimestamps={contributor.repositories.flatMap(
                      (repo: ContributorRepository) => repo.commit_timestamps
                    )}
                    prTimestamps={contributor.repositories.flatMap(
                      (repo: ContributorRepository) => repo.pr_timestamps
                    )}
                    globalStartDate={timelineDisplayBoundaries.startDate || undefined}
                    globalEndDate={timelineDisplayBoundaries.endDate || undefined}
                  />
        ...
        {timeWindow.preset !== 'all' && filteredSummaryMetrics?.activeContributors === 0 && (
          <div className={styles.noContributorsMessage}>
            <p>No contributors found with activity in the selected time window.</p>
            <p>
              Try selecting a different time period or &quot;All time&quot; to see all contributors.
            </p>
          </div>
        )}
      </div>
