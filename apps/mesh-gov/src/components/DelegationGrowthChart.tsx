...
export default function DelegationGrowthChart({ data }: DelegationGrowthChartProps) {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr(
        'class',
        `${styles.lineDelegators}${hovered === 'delegators' ? ' ' + styles.lineDelegatorsActive : ''}`
      )
      .attr('d', lineDelegators)
      .style('stroke', 'url(#gradientDelegators)');

    // Add hover effects
    const focus = svg.append('g').attr('class', styles.focus).style('display', 'none');

    focus.append('circle').attr('class', styles.focusCircleAda).attr('r', 4);

    focus.append('circle').attr('class', styles.focusCircleDelegators).attr('r', 4);
    ...
    overlay
      ...
      .on('mousemove', (event: MouseEvent) => {
        ...
        const bisectDate = d3.bisector<DelegationDataPoint, Date>(
          (d: DelegationDataPoint) => d.date
        ).left;
        ...
        const distanceToDelegators = Math.min(
          ...
          distToSegment(
            ...
            yDelegators,
            xScale(d1.date),
            yScaleDelegators(d1.totalDelegators)
          )
        );
        ...
        focus.select('.focusCircleAda').attr('transform', `translate(${x},${yAda})`);

        focus.select('.focusCircleDelegators').attr('transform', `translate(${x},${yDelegators})`);
  ...
  return (
    <div className={styles.container} style={{ position: 'relative' }}>
      ...
      <div className={styles.legendBox}>
        <div
          className={styles.legendItem + (hovered === 'ada' ? ' ' + styles.legendItemActive : '')}
          ...
          <div className={styles.tooltipContent}>
            {hovered && (
              <>
                ...
                <div className={styles.tooltipDate}>{hoveredData?.date.toLocaleDateString()}</div>
              </>
            )}
          </div>
        </div>
