...
export default function VotingTypeDonut({ typeStats }: VotingTypeDonutProps) {
  ...
  const drawChart = (isHovered: string | null) => {
    ...
    data.forEach(({ type, value }, idx) => {
      ...
      const stops =
        TYPE_GRADIENTS[GRADIENT_KEYS[idx % GRADIENT_KEYS.length] as keyof typeof TYPE_GRADIENTS];
          ...
          >
            <span
              ...
              style={{
                background:
                  TYPE_GRADIENTS[
                    GRADIENT_KEYS[idx % GRADIENT_KEYS.length] as keyof typeof TYPE_GRADIENTS
                  ][0],
              }}
            ></span>
